...
- **`Extent`** — an inlined quantitative measure of a dataset's size or scope, with a numeric `quantity` and multilingual `unit` labels. A dataset can carry several extents, such as record and file counts plus total bytes; each measure stays machine-readable instead of combining its number and unit in prose.
...
`Affiliation.affiliation_role` describes a person's organization-level status: employee, faculty, researcher, student, intern, founder, owner, advisor, contractor, volunteer, member, fellow or affiliate. Prefer the most specific role over the `EMPLOYEE` and `MEMBER` fallbacks, and create separate affiliation entries when distinct roles coexist or apply during different dates. Project responsibilities belong in `ProjectParticipation`, not `Affiliation`.
