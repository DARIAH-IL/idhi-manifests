...
3. **Reified relationships.** Any Person/Organization relationship that carries a role or dates is a class extending `Relationship` (endpoints + role enum + start/end dates), not a direct slot. Relationship classes have no `id` and are inlined.
